...
ID3D11Buffer*           Renderer::m_SpriteAnimationBuffer{};
...
void Renderer::Init()
{
	...
	bufferDesc.ByteWidth = sizeof(SPRITE_ANIMATION);


	m_Device->CreateBuffer(&bufferDesc, NULL, &m_SpriteAnimationBuffer);
	m_DeviceContext->VSSetConstantBuffers(6, 1, &m_SpriteAnimationBuffer);
...
void Renderer::Uninit()
{
	...
	m_SpriteAnimationBuffer->Release();
...
void Renderer::Begin()
{
	float clearColor[4] = { 0.2196f, 0.9137f, 0.5216f, 1.0f };
...
void Renderer::SetProjectionMatrix3D() //カメラのプロジェクション行列を使用することを推奨します
{
	XMMATRIX projection;
...
void Renderer::SetSpriteAnimation( SPRITE_ANIMATION animation )
{
	/*DebugLog(("Frame=" + std::to_string(animation.Frame)
			+ " Col=" + std::to_string(animation.Columns)
			+ " Row=" + std::to_string(animation.Rows)).c_str());*/
	m_DeviceContext->UpdateSubresource(m_SpriteAnimationBuffer, 0, NULL, &animation, 0, 0);
}
...
void Renderer::CreateVertexShader( ID3D11VertexShader** VertexShader, ID3D11InputLayout** VertexLayout, const char* FileName )
{
	...
	{
		OutputDebugStringA("Render > VertexShader > PATH: ");
		OutputDebugStringA(buf);
		OutputDebugStringA("\n");
	}
	else
	{
		OutputDebugStringA("Render > VertexShader > getcwdに失敗しました\n");
	}
...
void Renderer::CreatePixelShader( ID3D11PixelShader** PixelShader, const char* FileName )
{
	...
	if (_getcwd(buf, sizeof(buf)) != nullptr)
	{
		OutputDebugStringA("Render > PixelShader > PATH: ");
		OutputDebugStringA(buf);
		OutputDebugStringA("\n");
	}
	else
	{
		OutputDebugStringA("Render > PixelShader > getcwdに失敗しました\n");
	}
